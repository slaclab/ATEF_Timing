...
def find_static(list1: int, list2: int, PV: dict, pvlist_new: str, folder: str, ioc: str):
    ...
    if df1.shape == df2.shape:
        ...
        static_PV_value.to_csv(folder + ioc + '_static_PV_value.csv', index=False)
...
def configure(hutch: str, ioc: str, iocindex: int, progress: float, PV: dict, folder: str):
    ...
    if json_object["root"]["name"] != hutch:
        json_object["root"]["name"] = hutch
    ...
    if progress == 1:
        del json_object["root"]["configs"][0]
